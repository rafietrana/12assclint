...
const Reservation = () => {
  ...
  const { data: getreserve = [], refetch } = useQuery({
    ...
    queryFn: () =>
      axios
        .get("https://my-ass-12-server.vercel.app/getreserve")
        .then((res) => res.data),
  });
  ...
  const { data: emailSearchValue = [] } = useQuery({
    ...
    queryFn: () =>
      axios
        .get(`https://my-ass-12-server.vercel.app/searchbyemail/${emailValues}`)
        .then((res) => res.data),
  });

  useEffect(() => {
    if (emailValues && emailSearchValue.length > 0)
              ...
              >
                ...
                <td className="border border-gray-300 p-2">
                  <div className="avatar mx-auto">
                    <div className="w-16 h-16 rounded-full overflow-hidden">
                      <img
                        src={
                          reserveget?.userPhoto ||
                          "https://via.placeholder.com/64"
                        }
                        alt={reserveget?.userName || "User"}
                ...
                <td className="border border-gray-300 p-2">
                  <button
                    ...
                    className="bg-gradient-to-b from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-bold py-2 px-3 rounded-full shadow-md hover:shadow-lg transition duration-300 ease-in-out"
                  >
                    Cancel
                  </button>
                </td>
